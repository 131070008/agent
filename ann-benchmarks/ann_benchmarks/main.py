...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        dest="output_dir",
        default=None,
        help="Directory for result .hdf5 files. Filenames keep the normal ann-benchmarks parameter name and add _Nt from OMP_NUM_THREADS.",
    )
    parser.add_argument(
        "--index-dir",
        default=None,
        help="Root directory for reusable algorithm indexes. For Faiss HNSW, files are stored under INDEX_DIR/DATASET/.",
    )
    parser.add_argument(
        "--index-mode",
        choices=["none", "auto", "save", "load"],
        default="none",
        help="Reusable index mode: none disables cache; auto loads if present or builds+saves; save rebuilds+saves; load requires an existing index.",
    )
    parser.add_argument(
    ...
    for definition in definitions:
        not_yet_run = [
            ...
            if force or not os.path.exists(
                build_result_filepath(dataset, count, definition, query_args, batch, output_dir)
            )
        ]
