...
class HaystackRAGGenerator:
    """RAG generator using Haystack prompt building and OpenAI/Ollama LLM endpoints."""
    ...
    def __init__(
        ...
        Args:
            api_base_url: Base URL for OpenAI-compatible or Ollama API endpoint
            model_name: Model name/path to use for generation
            api_key: API key (use "EMPTY" for local endpoints)
            max_tokens: Maximum tokens to generate
            temperature: Sampling temperature
            provider: LLM endpoint type: auto, openai, or ollama
            num_ctx: Ollama context window size
            think: Enable Ollama reasoning mode when supported
        """
        ...
        self.think = think
        self.provider = self._resolve_provider(provider, api_base_url)
        self.generator = None
 
        # Test connection to the API endpoint
        try:
            if self.provider == "ollama":
                response = requests.get(f"{self._ollama_root()}/api/tags", timeout=5)
            else:
                response = requests.get(f"{self._openai_root()}/v1/models", timeout=5)
            if response.status_code == 200:
                print(f"✅ Connected to LLM endpoint: {api_base_url} ({self.provider})")
            else:
    ...
    @staticmethod
    def _resolve_provider(provider: str, api_base_url: str) -> str:
        if provider != "auto":
            return provider
        lower = api_base_url.lower()
        if "11434" in lower or "ollama" in lower or lower.endswith("/api/chat"):
            return "ollama"
        return "openai"
...
Answer:"""
    ...
    def _generate_with_ollama(self, prompt: str) -> str:
        ...
        response = requests.post(f"{self._ollama_root()}/api/chat", json=payload, timeout=None)
...
def main():
    ...
    def add_common(p):
        ...
        p.add_argument("--llm-provider", type=str, default="auto", choices=["auto", "openai", "ollama"],
                      help="LLM endpoint provider. auto treats port 11434 as Ollama")
        p.add_argument("--llm-num-ctx", type=int, default=4096,
                      help="Ollama context window size")
        p.add_argument("--llm-think", action="store_true",
                      help="Enable Ollama reasoning mode when supported. Default keeps think=false")
